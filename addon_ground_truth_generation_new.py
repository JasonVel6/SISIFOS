...
def _new_slot(node, name):
    # Blender ≥3.0 -----------------
    if hasattr(node, "file_slots"):
        slots = node.file_slots
    # Blender 2.83 fallback --------
    else:
        slots = node.layer_slots
    return slots.new(name)
...
def load_handler_render_init(scene):
    ...
    if scene.vision_blender.bool_save_gt_data:
        ...
        print("TMP_FILES_PATH =", TMP_FILES_PATH)
        print("abs TMP_FILES_PATH =", os.path.abspath(TMP_FILES_PATH))
        #input("paths")
        clean_folder(TMP_FILES_PATH)
        node_output.base_path = TMP_FILES_PATH

        # 3. Set-up links between nodes
        slots = getattr(node_output, "file_slots", node_output.layer_slots)
        slots.clear()
        ...
        """ Depth map """
        if vision_blender.bool_save_depth:
            slot_depth = _new_slot(node_output, "####_Depth")
...
def load_handler_after_rend_frame(
    scene,
):  # TODO: not sure if this is the best place to put this function, should it be above the classes?
    ...
    if scene.vision_blender.bool_save_gt_data:
        ...
        if vision_blender.bool_save_cam_param:
            ...
            if is_stereo_activated:
                ...
                if transf0to1 is not None:
                    extrinsic_mat0 = np.vstack((extrinsic_mat0, [0, 0, 0, 1.0]))
        ...
        save_data_to_npz(
            ...
            extrinsic_mat1,
            obj_poses,
        )  # Object poses are relative to the world coordinate frame, so they are the same
...
class RENDER_PT_gt_generator(GroundTruthGeneratorPanel):
    ...
    def draw(self, context):
        ...
        if context.engine != "CYCLES":
            ...
            col.label(
                text="Optical Flow and Segmentation Masks requires Cycles!",
                icon="ERROR",
            )
...
"""
TODO:
    - Check Blender Workbench engine
"""
