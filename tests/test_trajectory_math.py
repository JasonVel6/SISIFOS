...
from modules.trajectory.trajectory_math import R2q, q2R, qProd, sk, unsk
...
class TestTrajectoryMath:
    ...
    def test_quaternion_rotation_conversion(self):
        ...
        np.testing.assert_array_almost_equal(
            q, sign * q_recovered, err_msg="R2q did not recover the equivalent quaternion."
        )
    ...
    def test_so3_log_vec(self):
        ...
        np.testing.assert_array_almost_equal(
            log_map_result, expected_result, err_msg="so3_log_vec failed to map rotation matrix to rotation vector."
        )
