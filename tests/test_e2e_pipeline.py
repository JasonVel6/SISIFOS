...
class TestE2EPipeline:
    def test_full_pipeline_image_and_data_integrity(self):
        ...
        assert len(img_paths) == 4, (
            f"Expected 4 rendered frames, found {len(img_paths)}."
        )

        target_found, earth_found = False, False
        for img_path in img_paths:
            img = plt.imread(str(img_path))
            if np.sum(img[:, :, 0] > 0.8) > 0:  # Red
                target_found = True
            if np.sum(img[:, :, 2] > 0.8) > 0:  # Blue
                earth_found = True

        assert target_found, "Target (red pixels) not found in any frame."
        assert earth_found, (
            "Earth (blue pixels) not found in any frame. Orbit scaling or clipping failed."
        )
        ...
        if np.any(target_mask):
            ...
            assert 10.0 < mean_depth < 100.0, (
                f"Expected target depth in [10,100]m, got {mean_depth:.2f}m."
            )
