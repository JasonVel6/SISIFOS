...
from modules.trajectory.generateTrajectoriesUnified import (
...
def generate_trajectories(
    config: SceneConfig, output_dir: Path, config_prefix: str
) -> list[str]:
        ...
        for item in src_folder.iterdir():
            if item.is_file():
                ...
                if not dest.exists():
                    os.symlink(
                        item.resolve(), dest
                    )  # Create a symlink to avoid copying large folders
...
def run_sisfos_with_config(config: SceneConfig, renders_base_dir: Path):
    ...
    trajectory = get_scaled_trajectory_in_ECI(
        trajectory, earth_dist_scale_factor=config.render.earth_dist_scale_factor
    )
    ...
    with tqdm(total=total, desc=f"Rendering {model.name}") as pbar:
        ...
        for i in frame_ids:
            ...
            if (
                str(config.setup.enable_blur).casefold() == "on"
                and fake_fr2 is not None
            ):
                fps = renderer.scene.render.fps / renderer.scene.render.fps_base
                shutter_frames = (
                    config.camera.exposure_time_s
                    * fps
                    * config.setup.blur_shutter_factor
                )
    ...
    image_paths = [
        os.path.join("images", image_filename) for image_filename in image_filenames
    ]
...
def run_sweep(sweep_config: SweepConfig):
    ...
    for i, config in enumerate(configs):
        ...
        agent_folders = generate_trajectories(
            config, output_dir, config_prefix=config_prefix
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--config_path", type=str, help="Path to the base configuration JSON file"
    )
    parser.add_argument(
        "--sweep_config_path",
        type=str,
        help="Path to the sweep configuration JSON file (optional)",
    )
    ...
    if args.sweep_config_path:
        ...
        sweep_config_json = {
            ...
            "sweep_parameters": sweep_config_json,
        }
        sweep_config = SweepConfig.model_validate(sweep_config_json)
