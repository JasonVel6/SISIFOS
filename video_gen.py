...
def select_images(folder: Path, substring: str, glob_pattern: str) -> list[Path]:
    imgs = [
        p for p in folder.glob(glob_pattern) if p.is_file() and p.suffix.lower() in IMAGE_EXTS and substring in p.name
    ]
...
def output_path_for(base: Path, folder: Path, substring: str, output_dirname: str, fps: float) -> Path:
    ...
    if has_substring_component:
        ...
        stem = (
            f"{sanitize_for_filename(rel_str)}__{sanitize_for_filename(substring)}_fps_{sanitize_for_filename(fps_tag)}"
        )
    name = f"{stem}.mp4"
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--output-dir", default=OUTPUT_DIRNAME, help=f"Directory under base to place mp4s (default '{OUTPUT_DIRNAME}')."
    )
    p.add_argument(
        "--substrings", nargs="+", default=SUBSTRINGS, help=f"List of substrings to try (default: {SUBSTRINGS})."
    )
    return p.parse_args()
