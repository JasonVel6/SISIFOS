...
def select_images(folder: Path, substring: str, glob_pattern: str) -> List[Path]:
    imgs = [
        p
        for p in folder.glob(glob_pattern)
        if p.is_file() and p.suffix.lower() in IMAGE_EXTS and substring in p.name
    ]
...
def ensure_ffmpeg() -> None:
    try:
        ...
        raise RuntimeError(
            f"ffmpeg not available ({FFMPEG_BIN}). Install it or set env var FFMPEG_BIN. {e}"
        )


def run_ffmpeg_concat(
    list_file: Path, output_mp4: Path, fps: float, overwrite: bool
) -> None:
...
def output_path_for(
    base: Path, folder: Path, substring: str, output_dirname: str, fps: float
) -> Path:
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--base", type=Path, required=True, help="Base folder to scan recursively."
    )
    p.add_argument(
        "--fps",
        type=float,
    ...
    p.add_argument(
        "--overwrite", action="store_true", help="Overwrite existing mp4 outputs."
    )
    p.add_argument(
        "--output-dir",
...
def main() -> None:
    ...
    for folder in folders:
        for substring in args.substrings:
            ...
            out_mp4 = output_path_for(
                base, folder, substring, args.output_dir, args.fps
            )
            ...
            try:
                with tempfile.TemporaryDirectory() as td:
                    ...
                    print(
                        f"[RUN ] {rel_folder} | '{substring}' -> {out_mp4.relative_to(base)} "
                        f"(frames={len(imgs)}, fps={args.fps})"
                    )
                    run_ffmpeg_concat(
                        list_file, out_mp4, args.fps, overwrite=args.overwrite
                    )
                generated += 1
            except Exception as e:
                errors += 1
