...
class SetupConfig(BaseModel):
    ...
    t_ref_s: float = (
        0.01666667  # TODO why is this deffined here and in the camera config
    )
...
class TrajectoryConfig(BaseModel):
    ...
    m: float = 4000.0
    length: float = 5.0  # longest axis (5m), y=3.2m, z=2.8m
    w: float = 3.2
    h: float = 2.8
    ...
    def J(self) -> np.ndarray:
        ...
        J_yy = (1 / 12) * self.m * (self.length**2 + self.h**2)  # rotation about y
        J_zz = (1 / 12) * self.m * (self.length**2 + self.w**2)  # rotation about z
        return np.array([[J_xx, 0, 0], [0, J_yy, 0], [0, 0, J_zz]])

    @property
    def COV_R_ASTRO_APS3(self) -> List[List[float]]:
        return [
            [self.sigma_Rxy_aps3**2, 0, 0],
            [0, self.sigma_Rxy_aps3**2, 0],
            [0, 0, self.sigma_Rz_aps3**2],
        ]

    @property
    def COV_OMEGA_ASTRIX(self) -> List[List[float]]:
        return [
            [self.sigma_omega**2, 0, 0],
            [0, self.sigma_omega**2, 0],
    ...
    def COV_ACCEL_ASTRIX(self) -> List[List[float]]:
        return [
            [self.sigma_accel**2, 0, 0],
            [0, self.sigma_accel**2, 0],
            [0, 0, self.sigma_accel**2],
        ]
...
class SceneConfig(BaseModel):
    ...
    trajectory_type: Literal[
        "trajectory_generator", "sampling_trajectory", "filepath"
    ] = "trajectory_generator"
    trajectory_sampling: SamplingTrajectoryConfig = Field(
        default_factory=SamplingTrajectoryConfig
    )
    ...
    model_rotation_z_deg: float = (
        45.0  # Apply initial Z rotation, will be extended to X,Y
    )
    ...
    def _set_nested_attr(obj: Any, param_path: str, value: Any) -> None:
            ...
            if isinstance(target, dict):
                ...
                if not hasattr(target, part):
                    raise AttributeError(
                        f"Missing attribute '{part}' in path '{param_path}'"
                    )
        ...
        if isinstance(target, dict):
            ...
            if not hasattr(target, last):
                raise AttributeError(
                    f"Missing attribute '{last}' in path '{param_path}'"
                )
