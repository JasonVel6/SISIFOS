...
def sample_inertia_excited_omega_direction(
    rng, J, min_asymmetry_component=0.4, off_axis_min=0.3, max_retries=100
):
    """
...
def validate_omega_timeseries_excitation(
    omega_timeseries, dt_array=None, omega_dot_min=1e-5, off_axis_min=0.3
):
    """
...
def inertial_nmc(
    ...
    for i in range(num_mc):
        ...
        base_rng = (
            rngs_mc[i] if (rngs_mc is not None) else np.random.default_rng(12345 + i)
        )
        ...
        for a in range(num_agents):
            ...
            if (focal_length_px is not None) and (kf_dt is not None):
                ...
                bad, _ = detect_degeneracy(
                    r0, v0, focal_length_px, float(kf_dt), px_min, rho_max
                )
                if bad:
                    ...
                    bad2, stats2 = detect_degeneracy(
                        r1, v1, focal_length_px, float(kf_dt), px_min, rho_max
                    )
                    if bad2:
                        raise RuntimeError(
                            f"NMC IC unrecoverable: px={stats2['px']:.2f}, rho={stats2['rho']:.3f}"
                        )
            ...
            if (focal_length_px is not None) and (kf_dt is not None):
                ...
                if bad:
                    zN, vzN = repair_inertial_cro(
                        r0, v0, focal_length_px, float(kf_dt), n_scalar, px_min, vz0=vz0
                    )
                    ...
                    if bad2:
                        ...
                        bad3, stats3 = detect_degeneracy(
                            r2, v2, focal_length_px, float(kf_dt), px_min, rho_max
                        )
                        if bad3:
                            raise RuntimeError(
    ...
    for i in range(num_mc):
        ...
        for a in range(num_agents):
            ...
            if (focal_length_px is not None) and (kf_dt is not None):
                ...
                if bad:
                    fix = repair_hill(
                        r0, v0, n_scalar, focal_length_px, float(kf_dt), px_min
                    )
                    if fix is not None:
                        x_0[i, a], y_0[i, a], z_0[i, a] = (
                            fix["x0"],
                            fix["y0"],
                            fix["z0"],
                        )
                        ...
                        if bad2:
                            raise RuntimeError(
                                f"Hill IC unrecoverable: px={stats2['px']:.2f}, rho={stats2['rho']:.3f}"
                            )
        ...
        try:
            if J is not None:
                ...
                d, _ = sample_excited_omega_direction(
                    rng, off_axis_min=off_axis_min, max_retries=max_omega_retries
                )
