    ...
    if str(rotMode_Gframe).strip() == "1" and isinstance(variant_rCGG, dict) and R_IG_i is not None:
        for suf, r_var in variant_rCGG.items():
            ...
            if r_var.ndim != 3 or r_var.shape[2] != 3:
                continue
            label_base = (
                f"traj{mc_idx}_{'nmc' if suf.endswith('nmc') else 'cro' if suf.endswith('cro') else suf.strip('_')}"
            )
            for agent_idx in range(min(r_var.shape[0], len(relI_list))):
                ...
                ax_relI.plot(relI_var[:, 0], relI_var[:, 1], relI_var[:, 2], lw=1.8, linestyle="--", label=L)
    ...
    if str(rotMode_Gframe).strip() == "1" and isinstance(variant_rCGG, dict) and R_IG_i is not None:
        for suf, r_var in variant_rCGG.items():
            ...
            for agent_idx in range(min(r_var.shape[0], state_C_I.shape[0])):
                ...
                L = label_base if agent_idx == 0 else None
                ax_inr.plot(r_CO_I_var[:, 0], r_CO_I_var[:, 1], r_CO_I_var[:, 2], lw=1.8, linestyle="--", label=L)
        ax_inr.legend(loc="upper right", fontsize="small")
...
def plot_scene_frame(
    ...
    # Target position
    ax1.scatter(
        [target_loc[0]], [target_loc[1]], [target_loc[2]], c=target_color, s=100, marker="*", label="Target", zorder=5
    )

    # Camera position
    ax1.scatter([camera_loc[0]], [camera_loc[1]], [camera_loc[2]], c="blue", s=80, marker="^", label="Camera", zorder=5)
    ...
    ax1.quiver(
        0, 0, 0, sun_arrow[0], sun_arrow[1], sun_arrow[2], color="orange", arrow_length_ratio=0.1, lw=3, label="Sun dir"
    )
    ...
    # Sun direction
    ax3.arrow(
        0, 0, sun_arrow2[0], sun_arrow2[1], head_width=3, head_length=2, fc="orange", ec="orange", lw=2, label="Sun"
    )

    # Earth direction
    ax3.arrow(
        0, 0, earth_arrow[0], earth_arrow[1], head_width=3, head_length=2, fc="green", ec="green", lw=2, label="Earth"
    )
...
def plot_relative_pose_frame(
    ...
    for vec, color in zip(g_basis, g_colors, strict=False):
        ax.quiver(0.0, 0.0, 0.0, vec[0], vec[1], vec[2], color=color, linewidth=2.2, arrow_length_ratio=0.15)
    for vec, color in zip(c_basis, c_colors, strict=False):
        ax.quiver(
            p_cg_i[0],
            p_cg_i[1],
            p_cg_i[2],
...
def generate_scene_plots(output_dir: str,
    ...
    sun_dir_I = np.array([np.cos(el0_rad) * np.cos(az0_rad), np.cos(el0_rad) * np.sin(az0_rad), np.sin(el0_rad)])
