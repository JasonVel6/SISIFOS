...
def _draw_g_axes(ax, GXs, GYs, GZs, axis_length=None):
    ...
    GXs.append(np.array([0.0, L]))
    GYs.append(np.array([0.0, 0.0]))
    GZs.append(np.array([0.0, 0.0]))
    GXs.append(np.array([0.0, 0.0]))
    GYs.append(np.array([0.0, L]))
    GZs.append(np.array([0.0, 0.0]))
    GXs.append(np.array([0.0, 0.0]))
    GYs.append(np.array([0.0, 0.0]))
    GZs.append(np.array([0.0, L]))
...
def plot_trial_trajectories(
    ...
    for agent_idx in range(num_agents):
        ...
        abs_listX.append(rS[:, 0])
        abs_listY.append(rS[:, 1])
        abs_listZ.append(rS[:, 2])
    ...
    ax_relI.set_title(
        f"{_nice_title_for_mode(mc_idx, rotMode_Gframe)} — A-Centered Relative Paths"
    )
    ax_relI.set_xlabel("x_A [m]")
    ax_relI.set_ylabel("y_A [m]")
    ax_relI.set_zlabel("z_A [m]")
    ax_relI.grid(True, alpha=0.3)

    for agent_idx, rel in enumerate(relI_list):
        c = colors[agent_idx]
        ax_relI.plot(
            rel[:, 0], rel[:, 1], rel[:, 2], lw=1.6, color=c, label=f"Agent {agent_idx}"
        )
    ...
    if (
        str(rotMode_Gframe).strip() == "1"
        and isinstance(variant_rCGG, dict)
        and R_IG_i is not None
    ):
    ...
    ax_inr.set_title(
        f"{_nice_title_for_mode(mc_idx, rotMode_Gframe)} — Inertial Trajectories (I)"
    )
    ax_inr.set_xlabel("x_I [m]")
    ax_inr.set_ylabel("y_I [m]")
    ax_inr.set_zlabel("z_I [m]")
    ...
    ax_G.set_xlabel("x_G [m]")
    ax_G.set_ylabel("y_G [m]")
    ax_G.set_zlabel("z_G [m]")
    ...
    for agent_idx in range(num_agents):
        ...
        c = colors[agent_idx]
        ax_G.plot(
            rG[:, 0], rG[:, 1], rG[:, 2], lw=1.6, color=c, label=f"Agent {agent_idx}"
        )
    ...
    if (
        ...
        for suf, r_var in variant_rCGG.items():
            ...
            for agent_idx in range(min(r_var.shape[0], num_agents)):
                L = (
                    label_base if agent_idx == 0 else None
                )  # only first agent gets legend label
                ax_G.plot(
                    r_var[agent_idx, :, 0],
                    r_var[agent_idx, :, 1],
                    r_var[agent_idx, :, 2],
...
def plot_scene_frame(
    ...
    earth_lookat_angle = np.degrees(
        np.arccos(np.clip(np.dot(earth_dir_from_target, look_at_dir), -1, 1))
    )
    ...
    if sun_cam_angle_G is not None:
        ...
        sun_camera_angle = np.degrees(
            np.arccos(np.clip(np.dot(sun_dir, -look_at_dir), -1, 1))
        )
    ...
    ax1.set_zlabel("Z")
    sun_info = (
        f"az={sun_az_deg:.1f}° el={sun_el_deg:.1f}°"
        if sun_az_deg is not None
        else f"∠={sun_camera_angle:.1f}°"
    )
    ...
    # Target at center
    ax2.scatter(
        [0], [0], [0], c=target_color, s=150, marker="*", label="Target", zorder=5
    )
    ...
    look_arrow_rel = (
        -cam_rel / (np.linalg.norm(cam_rel) + 1e-9) * cam_to_target_dist * 0.5
    )
    ax2.quiver(
        cam_rel[0],
        cam_rel[1],
    ...
    for name, x in positions.items():
        ax4.scatter(
            [x], [0], c=colors[name], s=sizes[name], marker=markers[name], zorder=5
        )
...
def generate_scene_plots(
    ...
    print(
        f"    Sun: STATIC in inertial frame (from frame 0: az={sun_az_I[0]:.1f}° el={sun_el_I[0]:.1f}°)"
    )
    ...
    max_pcg_dist = (
        float(np.max(np.linalg.norm(p_cg_i_all, axis=1))) if n_frames > 0 else 1.0
    )
    ...
    for i in range(0, n_frames, every_n_frames):
        ...
        sun_cam_angle_body = np.degrees(
            np.arccos(np.clip(np.dot(sun_dir_body, cam_dir_body), -1, 1))
        )
