...
def read_camera_trajectory(file_path: str) -> dict[str, np.ndarray]:
    ...
    trajectory = {
        ...
        "q_I_G": np.array(
            [[float(r["q_I_G_w"]), float(r["q_I_G_x"]), float(r["q_I_G_y"]), float(r["q_I_G_z"])] for r in rows],
            dtype=float,
        ),
        "p_C_I": np.array([[float(r["p_C_I_x"]), float(r["p_C_I_y"]), float(r["p_C_I_z"])] for r in rows], dtype=float),
        "q_I_C": np.array(
            [[float(r["q_I_C_w"]), float(r["q_I_C_x"]), float(r["q_I_C_y"]), float(r["q_I_C_z"])] for r in rows],
            dtype=float,
        ),
...
def write_gtvalues(
    ...
    with open(gtvalues_filepath, "w") as f:
        ...
        trace_J = np.trace(J)
        J_norm = J / trace_J
        inertia_5 = np.array([J_norm[0, 0], J_norm[1, 1], J_norm[0, 1], J_norm[0, 2], J_norm[1, 2]])
        f.write("inertia_A = \n")
