...
class BlenderRenderer:
    ...
    def setup_total(self):
        ...
        addon_path = os.path.join(
            os.path.dirname(__file__), "addon_ground_truth_generation.py"
        )

        spec = importlib.util.spec_from_file_location(
            "vision_blender_addon", addon_path
        )
        ...
        if str(self.config.setup.stars_mode).casefold() == "off":
            ...
            img = bpy.data.images.load(
                bpy.path.abspath(self.config.hdri_path), check_existing=True
            )
    ...
    def select_model_to_render(self) -> bpy.types.Object:
        ...
        models = [
            o for o in bpy.data.objects if o.parent is None and o.name.startswith("RF_")
        ]
        selected_model = None
        for model in models:
        ...
        if not selected_model:
            raise ValueError(
                f"Selected model '{self.config.selected_model}' not found among RF_* objects."
            )
    ...
    def render_frame_v2(
        ...
        # Model pose
        tuple(math.degrees(a) for a in model.rotation_euler)
        ...
        # Camera pose
        tuple(math.degrees(a) for a in cam.rotation_euler)
        (model.location - cam.location).normalized()
        (model.location - cam.location).length
